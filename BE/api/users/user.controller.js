...
module.exports = {
  ...
  createAdminGoogle2FA: (req, res) => {
    let secret = speakeasy.generateSecret({
      length: 20,
      name: titleSite + " admin user 2FA auth",
    });
    ...
    QRCode.toDataURL(secret.otpauth_url, (err, image_data) => {
      return res.json({
        success: 1,
        qr: image_data,
        s: secret.base32, // save this value to your db for the admin user
      });
    });
  },
  ...
  unActiveAdminGoogle2FA: (req, res) => {
    const body = req.body;
    const token = body.t;
    getAdminSecret2FA((err, results) => {
      if (err) {
        console.log(err);
        return;
      }
      const secret = results;
      const tokenValidates = speakeasy.totp.verify({
        secret,
        encoding: "base32",
        token,
      ...
      if (tokenValidates) {
        Disabled2FA("admin@gmail.com", (err, results) => {
          if (err) {
            console.log(err);
            return;
          }
  ...
  activeAdminGoogle2FA: (req, res) => {
    ...
    if (tokenValidates) {
      updateAdminSecret2FA(obj, (err, result) => {
        if (err) {
          console.log(err);
          return;
        }
        if (!result) {
          return res.json({
            success: 0,
            message: "Failed to turn on admin 2FA auth",
          });
        }

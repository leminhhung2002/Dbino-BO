            ...
            db.query(
              ...
              (error, result, fields) => {
                ...
                SEND_THONG_BAO(
                  ...
                  result[0].email,
                  result[0].email,
                  "Thông báo hoa hồng VIP",
                  `Cấp dưới (${result[0].nick_name}) vừa mua vip. Và bạn nhận được ${amountDuocCong}$ hoa hồng.`
                );
                SEND_THONG_BAO(
                  "vip",
                  email,
...
module.exports = {
  ...
  checkCodeSecure: (data, callback) => {
    db.query(
      `select email from users where email = ? and (code_secure = ? or code_secure is NULL)`,
        ...
        if (results[0].money_usdt >= tongPhi) {
          ...
          db.query(
            ...
            (error, results, fields) => {
              ...
              Tele.sendMessRut(
                `🌟Người dùng ${data.nick_name} vừa thực hiện rút tiền VNĐ\nSử dụng lệnh dưới vào BOT để thực hiện lệnh KIỂM TRA và RÚT:`
              );
              Tele.sendMessRut(`ARES-CHECK check ${data.nick_name}`);
              ...
              //==== IN vào lịch sử
              db.query(
                `insert into trade_history (email, from_u, type_key, type, currency, amount, real_amount, bank, note, status, created_at)
                        values(?,?,?,?,?,?,?,?,?,?,now())`,
                [
                  data.email,
                  data.nick_name,
            ...
            if (!!results[0].hhttisMe) {
              ...
              if (lvVip <= 8) {
                if (totalDLVip == 3 && hhTuanNay >= 2000) {
                  ...
                  db.query(
                    `UPDATE users SET level_vip = 8 WHERE ref_code = ?`,
                    [refForMe]
                  );
                }
              }
  ...
  getListHisTradeWalletPage: (data, callback) => {
    ...
    db.query(
      ...
      [data.nick, "hh", count_per_page, next_offset > 0 ? next_offset : 0],
      (error, results, fields) => {
        if (error) {
          return callback(error);
        }
    ...
    db.query(
      ...
      (error, res, fields) => {
        ...
        db.query(
          ...
          [ref_id, "hhv", ref_id, "klgd", count_per_page, next_offset],
          (error, results, fields) => {
            if (error) {
              return callback(error);
            }
  ...
  updateAdminSecret2FA: (data, callback) => {
    db.query(
      `UPDATE users SET active_2fa = 1, secret_2fa = ?, code_secure = null WHERE email = ?`,
      [data.s, "admin@gmail.com"],
      (error, results, fields) => {
        if (error) {
          return error;
        }
  ...
  changeAccType: async (data, callback) => {
    if (data.type === 1) {
      await new Promise((resolve, reject) => {
        db.query(
          `SELECT COUNT(ref_code) as isParent FROM users WHERE ref_code = (select upline_id from users WHERE id = ?)`,
          [data.id],
          (err, res) => {
            if (err) {
              return reject(err);
            ...
            await Promise.all(
              result.map(async (ele) => {
                const res = await new Promise((resolve, reject) => {
                  db.query(
                    `SELECT sum(amount) as amount from trade_history WHERE email = ? AND type_key ='nt'`,
                    [ele.email],
                    (error, results) => {
                      if (error) {
                        return reject(error);
